...
beforeEach(() => {
  ...
  engine.create.mockImplementation(() => ({
    ...
    setProgress: engine.setProgress,
    setVisible: engine.setVisible,
  }));
  vi.stubGlobal(
...
describe("3D journey lifecycle", () => {
  ...
  it("keeps a readable source when WebGL cannot start", async () => {
    ...
    render(<IllustratedJourney />);
    enter();
    await waitFor(() =>
      expect(
        screen.queryByRole("navigation", { name: "Encoding journey chapters" }),
      ).not.toBeInTheDocument(),
    );
    expect(
      screen.getByRole("heading", { name: "Value of allotment" }),
    ).toBeVisible();
  });
  it("does not create a WebGL context for reduced-motion readers", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "A disagreement. A correction." }),
    ).toBeVisible();
    expect(
      screen.getByRole("heading", { name: "The graph, growing." }),
    ).toBeVisible();
  });
});
